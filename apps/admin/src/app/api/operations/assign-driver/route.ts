...
export async function POST(request: Request) {
  try {
    ...
    if (!canDispatch(booking.payment_status, booking.status)) {
      return NextResponse.json(
        { error: "Booking must be paid and awaiting dispatch before assignment" },
        { status: 400 }
      );
    }
    ...
    if (driver.status && !["approved", "active", "pending"].includes(driver.status)) {
      return NextResponse.json(
        { error: "Driver is not eligible for assignment" },
        { status: 400 }
      );
    }
    ...
    await supabase.from("audit_events").insert({
      ...
      entity_type: "booking",
      entity_id: bookingId,
      metadata: { driver_id: assignedDriverUserId, authMode: auth.mode },
    });
